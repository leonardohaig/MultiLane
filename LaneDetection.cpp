...
bool LaneDetection::initialize_Img(std::string& img_name) {
	...
	m_resImg = img_src.clone();
	if(m_resImg.channels() == 1 )
		cv::cvtColor(m_resImg, m_resImg, CV_GRAY2BGR);
...
//		for (int n = 0; n < lm.size(); n++)
//		{
//			cv::circle(img_testShowLine, lm[n].str_p, 1, cv::Scalar(0, 0, 255), 1, 8, 0);
//		}
//
...
void LaneDetection::seed_generation(bool verbose) {
	...
	if (verbose)
	{
		cv::Mat img_test_marking_seed = cv::Mat::zeros(img_size, CV_8UC3);
	...
	if (verbose) {
		cv::Mat img_test_raw_level_assoc = cv::Mat::zeros(img_size, CV_8UC3);
		for (int ii = 0; ii < marking_seed.size(); ++ii) {
			if (marking_seed[ii].flag < 0) {
				continue;
			}
...
void LaneDetection::graph_generation(bool verbose) {
	...
	cv::Mat img_test_graph = m_resImg.clone();
...
void LaneDetection::validating_final_seeds(bool verbose)
{
	...
	cv::Mat img_test_val = cv::Mat::zeros(img_size, CV_8UC3);
	...
	for (int ii = 0; ii < marking_seed.size(); ii++)
	{
		...
		for (int jj = 0; jj < marking_seed[ii].index.size(); ++jj)
		{
			...
			cv::line(m_resImg, lm[temp_i].str_p, lm[temp_i].end_p, cv::Scalar(b, g, r), 2, 8, 0);
		}

		//cv::Mat final1 = img_test_val.clone();
		...
		for (int yy = marking_seed[ii].str_p.y; yy < marking_seed[ii].end_p.y; ++yy)
		{
			...
			cv::circle(m_resImg,     dot_p, 1, cv::Scalar(0, 255, 0), 1, 8, 0);
		}
